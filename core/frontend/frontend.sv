...
#(
    ...
    // COMMIT next PC for branch instructions - COMMIT
    input logic [CVA6Cfg.VLEN-1:0] npc_commit_i,
    // is COMMIT instruction a control flow instruction - COMMIT
    input logic is_commit_cf_i,
    // COMMIT Hart ID - COMMIT
    input logic [CVA6Cfg.LOG2_HARTS-1:0] commit_hartid_i,
    // is commit instruction compressed - COMMIT
    input logic is_commit_compressed_i,
    // is commit instruction valid - COMMIT
    input logic commit_valid_i,
    // ISSUE PC - ISSUE
    input logic [CVA6Cfg.VLEN-1:0] pc_issue_i,
    // ISSUE Hart ID - ISSUE
    input logic [CVA6Cfg.LOG2_HARTS-1:0] issue_hartid_i,
    // is issue instruction compressed - ISSUE
    input logic is_issue_compressed_i,
    // is issue instruction valid - ISSUE
  ...
  // --------------------
  // PC recovery after flush
  // --------------------
  logic [CVA6Cfg.NrHarts-1:0][CVA6Cfg.VLEN-1:0] npc_after_commit_flush_d, npc_after_commit_flush_q, npc_after_mispredict_flush_d, npc_after_mispredict_flush_q;
  ...
  always_comb begin
    flush_realign = {CVA6Cfg.NrHarts{flush_i}};
    flush_realign[replay_hartid] |= replay;
    flush_realign[icache_hartid_q] |= bp_valid;
  end
  assign icache_dreq_o.kill_s1 = flush_i || (replay && replay_hartid == icache_dreq_o.hartid);
  // if we have a valid branch-prediction we need to only kill the last cache request
  // also if we killed the first stage we also need to kill the second stage (inclusive flush)
  assign icache_dreq_o.kill_s2 = icache_dreq_o.kill_s1 || (bp_valid && icache_hartid_q == icache_dreq_i.hartid);
  ...
  always_comb begin : npc_select
    ...
    if (npc_rst_load_q) begin
      ...
      npc_after_commit_flush_d = {CVA6Cfg.NrHarts{boot_addr_i}};
      npc_after_mispredict_flush_d = {CVA6Cfg.NrHarts{boot_addr_i}};
    end else begin
      fetch_address = npc_q[next_hart];
      // keep stable by default
      npc_d         = npc_q;
      npc_after_commit_flush_d = npc_after_commit_flush_q;
      npc_after_mispredict_flush_d = npc_after_mispredict_flush_q;
      if(commit_valid_i) begin // if branch instruction use sbe.result field instead of pc_commit_i. 
        npc_after_commit_flush_d[commit_hartid_i] = is_commit_cf_i ? npc_commit_i : pc_commit_i + ((is_commit_compressed_i) ? {{CVA6Cfg.VLEN-2{1'b0}}, 2'h2} : {{CVA6Cfg.VLEN-3{1'b0}}, 3'h4});
      end
      if(resolved_branch_i.valid) begin
        npc_after_mispredict_flush_d[resolved_branch_i.hartid] = resolved_branch_i.target_address;
      end
      if(issue_valid_i) begin
        npc_after_mispredict_flush_d[issue_hartid_i] = pc_issue_i + ((is_issue_compressed_i) ? {{CVA6Cfg.VLEN-2{1'b0}}, 2'h2} : {{CVA6Cfg.VLEN-3{1'b0}}, 3'h4});
      end
    end
    // 0. Branch Prediction
    if (bp_valid) begin
    ...
    if (is_mispredict) begin
      npc_d = npc_after_mispredict_flush_q;
      npc_d[resolved_branch_i.hartid] = resolved_branch_i.target_address;
    end
    // 4. Return from environment call
    if (eret_i) begin
      npc_d = npc_after_commit_flush_q;
      npc_d[commit_hartid_i] = epc_i[commit_hartid_i];
      npc_after_commit_flush_d[commit_hartid_i] = epc_i[commit_hartid_i];
      npc_after_mispredict_flush_d = npc_after_commit_flush_d;
    end
    // 5. Exception/Interrupt
    if (ex_valid_i) begin
      npc_d = npc_after_commit_flush_q;
      npc_d[commit_hartid_i] = trap_vector_base_i[commit_hartid_i];
      npc_after_commit_flush_d[commit_hartid_i] = trap_vector_base_i[commit_hartid_i];
      npc_after_mispredict_flush_d = npc_after_commit_flush_d;
    end
    ...
    if (set_pc_commit_i) begin
      npc_d = npc_after_commit_flush_q;
      npc_d[commit_hartid_i] = pc_commit_i + {{CVA6Cfg.VLEN - 3{1'b0}}, 3'b100};
      npc_after_commit_flush_d[commit_hartid_i] = pc_commit_i + {{CVA6Cfg.VLEN - 3{1'b0}}, 3'b100};
      npc_after_mispredict_flush_d = npc_after_commit_flush_d;
    end
    // 7. Debug
  ...
  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
      ...
      bht_q             <= '0;
      icache_hartid_q   <= '0;
      npc_after_commit_flush_q <= '0;
      npc_after_mispredict_flush_q <= '0;
    end else begin
      npc_rst_load_q <= 1'b0;
      npc_q          <= npc_d;
      npc_after_commit_flush_q <= npc_after_commit_flush_d;
      npc_after_mispredict_flush_q <= npc_after_mispredict_flush_d;
