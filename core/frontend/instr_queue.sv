...
#(
  ...
  logic [CVA6Cfg.INSTR_PER_FETCH-1:0] instr_data_out_valid;
  ...
  assign fetch_entry_valid_o[0] = CVA6Cfg.MultihartEn ? instr_data_out_valid[idx_ds[0][1]] : ~(&instr_queue_empty);
  ...
  for (genvar i = 0; i < CVA6Cfg.NrIssuePorts; i++) begin
    ...
    assign fetch_entry_fire[i]  = fetch_entry_valid_o[i] & fetch_entry_ready_i[i] | fetch_entry_flushed;
  end
  assign fetch_entry_flushed = !instr_queue_empty[idx_ds[0][1]] && !instr_data_out_valid[idx_ds[0][1]];
  ...
  always_comb begin
    ...
    if (fetch_entry_fire[0] && !fetch_entry_flushed) begin
    ...
    if (valid_i[0] && reset_address_q[hartid_i]) begin
      ...
      reset_address_d[hartid_i] = CVA6Cfg.MultihartEn ? flush_i[hartid_i] : 1'b0;
    end
  end
  ...
  for (genvar i = 0; i < CVA6Cfg.INSTR_PER_FETCH; i++) begin : gen_instr_fifo
    ...
    cva6_fifo_mt #(
        ...
        .valid_o   (instr_data_out_valid[i]),
        .pop_i     (pop_instr[i])
    );
  end else begin
    cva6_fifo_v3 #(
