...
module scoreboard #(
  ...
  always_comb begin : issue_fifo
    ...
    if (CVA6Cfg.MultihartEn) begin
      ...
      for (int unsigned i = 0; i < CVA6Cfg.NR_SB_ENTRIES; i++) begin
        commit_pointer_n[0] = i + commit_pointer_q[0] + num_commit;
        if (!mem_n[commit_pointer_n[0]].flushed) break;
        mem_n[commit_pointer_n[0]].issued    = 1'b0;
        mem_n[commit_pointer_n[0]].flushed   = 1'b0;
      end
    end else begin
    if (flush_i) begin
      for (int unsigned i = 0; i < CVA6Cfg.NR_SB_ENTRIES; i++) begin
