...
module cva6_fifo_mt #(
    ...
    output logic                  flushed_o,   // outpud data has been flushed
  ...
  always_comb begin : read_write_comb
    ...
    for (int i = 0; i < FifoDepth; i++) begin
        mem_n[i].flushed |= flush_i[mem_n[i].data.hartid]; 
    end
    flushed_o  = (DEPTH == 0) ? flush_i[data_i.hartid] : mem_n[read_pointer_q].flushed;
    if (|flush_i)
      gate_clock = 1'b0;
