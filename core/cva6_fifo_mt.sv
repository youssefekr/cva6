...
module cva6_fifo_mt #(
    ...
    parameter int unsigned NrHarts = 1,  // number of harts
    // DO NOT OVERWRITE THIS PARAMETER
    parameter int unsigned ADDR_DEPTH = (DEPTH > 1) ? $clog2(DEPTH) : 1
) (
    input  logic                  clk_i,       // Clock
    input  logic                  rst_ni,      // Asynchronous reset active low
    input  logic [NrHarts-1:0]                 flush_i,     // flush the queue
    ...
    output logic                  valid_o,   // outpud data has been flushed
  ...
  // actual memory
  localparam type mem_t = struct packed {
    dtype data;
    logic valid;
};
  mem_t [FifoDepth - 1:0] mem_n, mem_q;
  ...
  always_comb begin : read_write_comb
    ...
    if (FPGA_EN) begin
      ...
      data_o     = (DEPTH == 0) ? data_i : mem_q[read_pointer_q].data;
      mem_n      = mem_q;
      gate_clock = 1'b1;
    end
    ...
    if (push_i && ~full_o) begin
      if (FPGA_EN) begin
        ...
        mem_n[write_pointer_q].data = data_i;
        mem_n[write_pointer_q].valid = 1'b1;
    ...
    for (int i = 0; i < FifoDepth; i++) begin
        mem_n[i].valid &= !flush_i[mem_n[i].data.hartid]; 
    end
    valid_o  = (DEPTH == 0) ? !flush_i[data_i.hartid] : mem_q[read_pointer_q].valid;
    if (|flush_i)
      gate_clock = 1'b0;
